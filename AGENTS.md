...
- **macOS gateway/app restarts:** The gateway runs as a separate daemon managed via launchd (`com.clawdbot.gateway`). To restart: **gateway daemon** → `clawdbot daemon restart`; **app only** → quit menubar icon then `open /Applications/Clawdbot.app`; **full rebuild** → use `/build:mac-clean` slash command. To verify processes: `launchctl print gui/$UID | grep clawdbot`. **When debugging, start/stop via the app or CLI, not ad-hoc tmux sessions; kill temporary tunnels before handoff.**
